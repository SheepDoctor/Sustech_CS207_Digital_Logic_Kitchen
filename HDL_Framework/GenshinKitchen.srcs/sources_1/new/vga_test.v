...
/*
Author: Zhao
Description: Transform the datas to the vga data to present.
*/
module vga_test(
input 					clk,
input 					nrst,
...
always @(posedge clk or negedge nrst) begin
	if(!nrst) begin
		data <= `blue;
			...
			if(x > `startx & x <= `startx + `wide & y > `line1 & y <= `line1 + `hight) begin //Show dataIn_bits
        ...
        else if(x > `startx & x <= `startx + `wide & y > `line2 & y <= `line2 + `hight) begin //Show signal
        ...
        else if(x > `startx & x <= `startx + `wide & y > `line3 & y <= `line3 + `hight) begin //Show pc
                    if(pc[7]) data <= `white;
                    else data <= `black;
               end
        ...
        else if(x > `startx & x <= `startx + `swide & y > `line4 & y <= `line4 + `hight) begin //Show script
			...
			data <= `blue; // The background color of the screen.
	end
end
endmodule 
